import styles from '../../../../styles/Home/projects/carouselMobile.module.css';
...
const projects = [
    ...
    },
    {
        ...
        description: "No início da minha carreira como freelancer, desenvolvi um sistema de concursos online com provas geradas aleatoriamente e realizadas em uma sala virtual. O projeto é exclusivo da empresa e não está disponível para visualização."
    },
    {
        title: "Brindear",
        role: "Freelancer",
    ...
    return (
        <div className={styles.cardsMobile}>
            ...
            {projects.slice(currentIndex, currentIndex + cardsToShow).map((project, index) => (
                <div key={index} className={styles.cardMobile} style={{ backgroundImage: `url(${project.image.src})` }}>
                    {project.role && <p className={styles.infoProjectMobile}>{project.role}</p>}
                    <div className={styles.headerContainer}>
                        <p className={styles.cardTitleMobile}>{project.title}</p>
                        <div className={styles.technologiesMobile}>
                            {project.technologies.map((tech, i) => (
                                <p key={i} className={styles.technologieMobile}>{tech}</p>
                            ))}
                        </div>
                    </div>
                        <p className={styles.projectInfoMobile}>{project.description}</p>
                        {project.link && <a href={project.link} target="_blank" rel="noopener noreferrer" className={styles.cardButtonMobile}>Visualizar Projeto</a>}
                </div>
            ))}
